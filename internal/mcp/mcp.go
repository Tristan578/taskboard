	...
	return []toolDef{
		...
		},
		{
			...
			Description: "Create a ticket (task) within a project. Tickets are concrete, actionable units of work. " +
				...
				"If the project uses Strict Mode, non-draft tickets require userStory and acceptanceCriteria.",
			InputSchema: jsonSchema{
				Type: "object",
				Properties: map[string]schemaProp{
					...
					"acceptanceCriteria": {Type: "string", Description: "Acceptance criteria (Given/When/Then)"},
					"technicalDetails":   {Type: "string", Description: "Technical implementation details"},
					"testingDetails":     {Type: "string", Description: "Testing plan details"},
					"isDraft":            {Type: "boolean", Description: "Create as draft (bypasses strict mode validation)"},
				},
		...
		{
			Name:        "update_ticket",
			Description: "Update ticket properties. Use this to add userStory and acceptanceCriteria to satisfy Strict Mode requirements.",
			InputSchema: jsonSchema{
				Type: "object",
				Properties: map[string]schemaProp{
